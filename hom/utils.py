...
def get_role_by_name(guild: discord.Guild, role_name_to_find: str) -> t.Optional[discord.Role]:
    roles = guild.roles
...
def contains_roles(roles: t.List[discord.Role], *accepted_roles: discord.Role) -> bool:
    lower_roles = [r.name.lower() for r in roles]
    return any(str(r).lower() in lower_roles for r in accepted_roles) or not accepted_roles
...
def get_category(guild: discord.Guild, channel_name: str) -> t.Optional[discord.CategoryChannel]:
    ...
    return discord.utils.find(
        lambda c: str(c.name).lower().replace("-", " ") == channel_name,
        guild.categories,  # JONX_TODO: Changed this to categories, is that right?
    )
...
def get_channel_by_name(bot, discord_guild_id: int, channel_name: str, channel_type: str = "text"):
    ...
    return discord.utils.find(
        lambda c: str(c.name).lower().replace("-", " ")
        == str(channel_name).lower().replace("-", " ")
        and str(c.type).lower() == channel_type,
        bot.get_guild(int(discord_guild_id)).channels,
    )
...
def get_user_ticket_channel(guild, user):
    category = get_category(
        discord_guild=guild, channel_name="Tickets"
    )  # TODO: Make this channel_id?
...
async def create_ticket_for_user(interaction, instructions, button_label, example_url=None):
    ...
    tickets_category = get_category(
        discord_guild=interaction.guild, channel_name="Tickets"
    )  # TODO: Make this channel_id?
    ...
    embed.set_footer(
        text="This channel is only visible to you and our moderators. If your question has been answered, feel free to close the ticket."
    )
    await interaction.followup.send(content=msg_content, ephemeral=True)
    await new_text_channel.send(
        content=f"{interaction.user.mention}", embed=embed, view=vw.PViewSupport_Message()
    )
...
async def send_log_message(interaction, content, mod, channel=None):
    ...
    return (
        await mod_logs_channel.send(embed=embed, file=f if channel else None)
        if mod_logs_channel
        else None
    )
...
async def archive_channel_messages(channel_being_archived):
    ...
    try:
        for message in [
            message
            async for message in channel_being_archived.history(limit=None, oldest_first=True)
        ]:
