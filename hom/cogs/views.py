...
class PViewSupport(discord.ui.View):
    ...
    async def api_key(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
...
class PViewVerify(discord.ui.View):
    ...
    async def verify_yes(
        ...
        await interaction.response.send_message(
            content=f"{cd.status_emoji_complete} Closing this ticket.", ephemeral=True
        )
        await interaction.channel.delete()


class PViewSupportGroup(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
    ...
    async def group_verify(
        ...
        await du.create_ticket_for_user(
            interaction,
            instructions,
            f"Group {cd.ARROW} " + button.label,
    ...
    async def group_other(
        ...
        instructions = f"Explain what you require assistance with below.\n\n{cd.view_footer}"
        await du.create_ticket_for_user(
            interaction, instructions, f"Group {cd.ARROW} " + button.label
        )


class PViewSupportNames(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
    ...
    async def names_approve(
        ...
        await du.create_ticket_for_user(
            interaction, instructions, f"Names {cd.ARROW} " + button.label
        )
...
class PViewSupportMessage(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
    ...
    async def message_close(
        ...
        await interaction.followup.send(
            ephemeral=False, embed=embed, view=PViewSupport_Message_Close_Channel()
        )


class PViewSupportMessageCloseChannel(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
...
async def setup(bot: commands.Bot) -> None:
    bot.add_view(PViewSupport())
    bot.add_view(PViewVerify())
    bot.add_view(PViewSupportGroup())
    bot.add_view(PViewSupportNames())
    bot.add_view(PViewSupportMessage())
    bot.add_view(PViewSupportMessageCloseChannel())
